...
public class CosFunction implements IFunction {
	...
	public List<ExpressionValue> evaluate(List<ExpressionValue> params, int numOutputs) throws WorkspaceInputException {
		if(numOutputs > Function.COS.getMaxOutputs()) throw new WorkspaceInputException("Too many outputs for function 'cos'.");
		if(params.size() != 1) throw new WorkspaceInputException("Invalid parameters for function 'cos'.");
		List<ExpressionValue> ret = new ArrayList<ExpressionValue>();
		ret.add(MatOp.cos(params.get(0)));
		return ret;
	}
