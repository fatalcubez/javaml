...
public class SinFunction implements IFunction {
	...
	public List<ExpressionValue> evaluate(List<ExpressionValue> params, int numOutputs) throws WorkspaceInputException {
		if(numOutputs > Function.SIN.getMaxOutputs()) throw new WorkspaceInputException("Too many outputs for function 'sin'.");
		if(params.size() != 1) throw new WorkspaceInputException("Invalid parameters for function 'sin'.");
		List<ExpressionValue> ret = new ArrayList<ExpressionValue>();
		ret.add(MatOp.sin(params.get(0)));
		return ret;
	}
