...
public class ExpFunction implements IFunction {
	...
	public List<ExpressionValue> evaluate(List<ExpressionValue> params, int numOutputs) throws WorkspaceInputException {
		if(numOutputs > Function.EXP.getMaxOutputs()) throw new WorkspaceInputException("Too many outputs for function 'exp'.");
		if(params.size() != 1) throw new WorkspaceInputException("Invalid parameters for function 'exp'.");
		List<ExpressionValue> ret = new ArrayList<ExpressionValue>();
		ret.add(MatOp.exp(params.get(0)));
		return ret;
	}
